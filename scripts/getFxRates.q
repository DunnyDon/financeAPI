...
getLatestEuroRates:{
 ...
 data:.j.k raze system raze"curl http://api.exchangeratesapi.io/v1/latest?access_key=",ccys;
...
getHistEuroRates:{[sd;ed;ccy]
 $[all null ccy;ccys:"";ccys:"&symbols=",csv sv string ccy];
  sd:"-" sv "." vs string sd;
  ed:"-" sv "." vs string ed;
 data:.j.k raze system raze"curl -X GET \"http://api.exchangeratesapi.io/v1/time-series?access_key=?start_at=",sd,"&end_at=",ed,ccys,"\"";
...
getLatestFxRates:{[base;ccy]
  $[all null ccy;ccys:"";ccys:"&symbols=",csv sv string ccy];
 data:.j.k raze system raze"curl -X GET \"http://api.exchangeratesapi.io/v1/latest?access_key=?base=",base,ccys,"\"";
...
getHistFxRates:{[base;sd;ed;ccy]
 $[all null ccy;ccys:"";ccys:"&symbols=",csv sv string ccy];
  sd:"-" sv "." vs string sd;
  ed:"-" sv "." vs string ed;
 data:.j.k raze system raze"curl -X GET \"http://api.exchangeratesapi.io/v1/time-series?access_key=?base=",base,"&start_at=",sd,"&end_at=",ed,ccys,"\"";
 data:flip `ccy`date`rate!raze each (enlist(count data`rates)#/:key flip data`rates),({(count cols value x)#/:key x};{value flip value x})@\:data`rates;
